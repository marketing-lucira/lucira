...
SELECT
  ...
  CASE
    ...
    WHEN f.on_hand>0 AND IFNULL(f.days_since_last_sale,9999)>270 AND f.inventory_value>=100000 THEN CONCAT('₹',CAST(CAST(ROUND(f.inventory_value) AS INT64) AS STRING),' idle ',CAST(f.days_since_last_sale AS STRING),'d')
  ...
  CASE
    ...
    WHEN f.on_hand>0 AND IFNULL(f.days_since_last_sale,9999)>DEAD_DAYS THEN CONCAT('Frees ₹',CAST(CAST(ROUND(f.inventory_value) AS INT64) AS STRING),' locked capital')
