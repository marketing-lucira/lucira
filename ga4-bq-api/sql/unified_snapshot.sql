-- ═══════════════════════════════════════════════════════════════════════════
-- UNIFIED GA4 snapshot — one JSON object carrying BOTH data models:
--   session : from ga4_dashboard.ga4_fact_sessions  (session grain, 200-day window)
--             → feeds every existing GA4 tab (Overview, Insights, Sales, Traffic,
--               Users, Devices, Locations, Pages, Events, E-commerce, Marketing, Explorer)
--   mart    : from ga4_data.ga4_data_part            (marketing × product mart, 90-day window)
--             → feeds the Merchandising tab + product-master enrichments
-- The two keep separate windows/metrics on purpose (they measure the same GA4 export
-- at different grains and would otherwise disagree on shared keys). Namespacing them
-- means no key collides and each tab reads the model it was built for.
--
-- Run standalone (regenerates gs://.../ga4/unified-latest-*.json):
--   bq --location=asia-south1 query --use_legacy_sql=false < unified_snapshot.sql
-- Automated daily: see unified_procedure.sql (rebuilds mart mirror + calls this export).
-- ═══════════════════════════════════════════════════════════════════════════
DECLARE sd_to   DATE DEFAULT DATE_SUB(CURRENT_DATE('Asia/Kolkata'), INTERVAL 1 DAY);
DECLARE sd_from DATE DEFAULT DATE_SUB(CURRENT_DATE('Asia/Kolkata'), INTERVAL 200 DAY);  -- session window
DECLARE md_to   DATE DEFAULT DATE_SUB(CURRENT_DATE('Asia/Kolkata'), INTERVAL 1 DAY);
DECLARE md_from DATE DEFAULT DATE_SUB(CURRENT_DATE('Asia/Kolkata'), INTERVAL 90 DAY);   -- mart window
...
WITH
  ...
  -- ── mart source + derived aggregates (sessions fan out → de-dup per traffic group) ──
  base AS (
